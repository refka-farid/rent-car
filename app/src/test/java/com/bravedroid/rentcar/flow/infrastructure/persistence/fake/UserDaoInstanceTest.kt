...
class UserDaoInstanceTest {
    ...
    @Before
    fun setUp() {
        dbMock = mock(MutableMap::class.java) as MutableMap<Long, UserEntity>
    ...
    fun `when db contains user UserDao should update it `() = runBlockingTest {
        ...
        `when`(dbMock[100L]).thenReturn(userEntity1)
        //act
        val result1 = sut.insertOrUpdate(user1).first()
        //assert
        assertThat(result1).isEqualTo(userEntity1)
    }
    ...
    fun `when db doesn't contains user UserDao should create it `() = runBlockingTest {
        ...
        //act
        val result1 = sut.insertOrUpdate(user2).first()
        //assert
        assertThat(result1).isEqualTo(userEntity2)
    }
    ...
    fun `when db contains user UserDao should delete it `() = runBlockingTest {
        ...
        //act
        val result = sut.delete(user1).first()
